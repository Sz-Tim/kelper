...
if(rerun_modelFits) {
  ...
  wtFrond_areaFrond.PAR_atDepth <- readRDS("temp/brms_wtFrond_areaFrond_PARdepth.rds")
  areaFrond_wtFrond.PAR_atDepth <- readRDS("temp/brms_areaFrond_wtFrond_PARdepth.rds")
}
...
if(rerun_modelFits) {
  ...
  saveRDS(depth_FAI, "temp/brms_PARdepth_FAI_K.rds")
} else {
  PAR_atDepth_FAI <- readRDS("temp/brms_PARdepth_FAI_K.rds")
}
...
if(rerun_modelFits) {
  ...
  depth_N <- readRDS("temp/brms_PARdepth_N_K.rds")
}
...
data.ls$month_biomass <- bind_rows(
  ...
  mutate(startDate=as_date(paste0("2017-", Month, "-01")),
         totalChg=biomassChg*days_in_month(startDate))

data.ls$month_biomass %>%
  ggplot(aes(startDate, totalChg, colour=Location)) + 
...
data.ls$month_biomass %>% 
  ggplot(aes(Sample, totalChg, colour=type)) + geom_point() +
  facet_wrap(~Species)
...
pars <- setParameters(
  prFullHarvest=0.8, freqHarvest=10, tmax=50, harvestTarget="stipe",
...
# Simulate population
```{r simulate_single}
out <- simulatePopulation_2Seasons(pars)
...
out.df %>% filter(stage=="canopy") %>%
  ggplot(aes(date, FAI, colour=factor(month))) + geom_line(size=1) + 
...
library(furrr)
plan(multisession)
out <- future_map(1:480, ~simulatePopulation_2Seasons(pars), 
