...
out.ls <- foreach(i=1:nrow(grid.i), 
  ...
  out <- map(1:3, ~simulatePopulation(par_i, lmType=lmType, ndraws=20))
...
mass.sum <- mass.df %>% group_by(id, year, month, date) %>%
  ...
  summarise(biomass_q10=quantile(biomass, 0.1),
            # biomass_q25=quantile(biomass, 0.25),
            # biomass_q75=quantile(biomass, 0.75),
...
mass.sum.sf %>% 
  filter(month==6) %>%
  group_by(id) %>%
  summarise(biomass=mean(biomass)/1e3) %>%
  ggplot(aes(fill=biomass)) + 
  geom_sf(colour=NA) + scale_fill_viridis_c() +
